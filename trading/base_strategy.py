...
class BaseStrategy(ABC):
    ...
    def get_market_phase(self, symbol: str, timeframe: int = mt5.TIMEFRAME_M15) -> MarketPhase:
        """
        Bestimme Marktphase.
        Kann in Subclasses überschrieben werden.
        """
    ...
    def _detect_swings(self, highs: np.ndarray, lows: np.ndarray, period: int = 5) -> Tuple[List, List]:
        ...
        for i in range(period, len(highs) - period):
            # Bullisher Swing
            if highs[i] > max(highs[i-period:i]) and highs[i] > max(highs[i+1:i+period+1]):
                swing_highs.append((i, highs[i]))
            
            # Bearischer Swing
            if lows[i] < min(lows[i-period:i]) and lows[i] < min(lows[i+1:i+period+1]):
    ...
    def _calculate_rr_ratio(self, entry: float, sl: float, tp: float, action: str) -> float:
