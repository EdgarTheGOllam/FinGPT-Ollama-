...
class RiskManager:
    ...
    def calculate_position_size(
        self, symbol, stop_loss_distance, account_risk_percent=None
    ):
        """
        ...
        try:
            ...
            self.log(
                "INFO",
                f"Position Size für {symbol}: {lot_size} Lots (Risiko: {account_risk_percent}%, SL: {stop_loss_distance} Pips)",
            )
    ...
    def can_open_position(self, symbol, action, lot_size):
        ...
        try:
            ...
            if self.daily_pnl <= self.max_daily_loss:
                return (
                    False,
                    f"Tägliches Verlustlimit erreicht ({self.daily_pnl:.2f}€ / {self.max_daily_loss:.2f}€)",
                )
            ...
            if symbol_info:
                ...
                ):  # Direkte Prüfung: benötigt > verfügbar
                    return (
                        False,
                        f"Nicht genügend Margin (benötigt: {required_margin:.2f}, verfügbar: {free_margin:.2f})",
                    )
            ...
            if (
                existing_positions
                and len(existing_positions) >= self.max_positions_per_symbol
            ):
                return (
                    False,
                    f"Max Positionen pro Symbol erreicht ({len(existing_positions)}/{self.max_positions_per_symbol})",
                )
            ...
            if all_positions and len(all_positions) >= self.max_total_positions:
                return (
                    False,
                    f"Max Gesamtpositionen erreicht ({len(all_positions)}/{self.max_total_positions})",
                )
            ...
            if self.trades_today >= self.max_trades_per_day:
                return (
                    False,
                    f"Max Trades pro Tag erreicht ({self.trades_today}/{self.max_trades_per_day})",
                )
            ...
            if self.last_trade_time:
                ...
                if time_since_last < self.min_time_between_trades:
                    ...
                    return (
                        False,
                        f"Warten Sie noch {remaining:.0f}s bis zum nächsten Trade",
                    )
            ...
            if not (self.trading_start_hour <= current_hour <= self.trading_end_hour):
                return (
                    False,
                    f"Außerhalb der Trading-Zeiten ({self.trading_start_hour}:00 - {self.trading_end_hour}:00)",
                )
            ...
            if not (self.min_lot_size <= lot_size <= self.max_lot_size):
                return (
                    False,
                    f"Lot-Größe außerhalb der Limits ({lot_size} nicht in {self.min_lot_size}-{self.max_lot_size})",
                )
    ...
    def register_trade(self, symbol, action, lot_size, result):
        ...
        try:
            ...
            self.log(
                "INFO", f"Trade registriert: {action} {lot_size} {symbol} - {result}"
            )
    ...
    def print_risk_status(self):
        ...
        try:
            ...
            print("─" * 40)
            print(
                f"Tages P&L: {summary.get('daily_pnl', 0):.2f}€ / {self.max_daily_loss:.2f}€"
            )
            print(
                f"Trades heute: {summary.get('trades_today', 0)} / {self.max_trades_per_day}"
            )
            print(
                f"Offene Positionen: {summary.get('open_positions', 0)} / {self.max_total_positions}"
            )
            ...
            pnl_status = (
                "🟢"
                if summary.get("daily_pnl", 0) > 0
                else "🔴"
                if summary.get("daily_pnl", 0) < self.max_daily_loss * 0.5
                else "🟡"
            )
            trades_status = (
                "🟢"
                if summary.get("trades_today", 0) < self.max_trades_per_day * 0.8
                else "🟡"
            )
            positions_status = (
                "🟢"
                if summary.get("open_positions", 0) < self.max_total_positions
                else "🟡"
            )
