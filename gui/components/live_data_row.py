...
class LiveDataRow(ctk.CTkFrame):
    ...
    def __init__(self, master, symbol, price, change, signal, 
                 min_height=None, compact_mode=False, **kwargs):
        self.default_color = "transparent"
        self.hover_color = "#2A2D34"
        ...
        super().__init__(master, fg_color=self.default_color, corner_radius=8, 
                        height=self._min_height, **kwargs)
        
        # Grid Setup for consistent column widths
        # Verwende weight für flexible Aufteilung, aber mit Mindestbreiten
        self.grid_columnconfigure(0, weight=0, minsize=self.MIN_WIDTHS['symbol'])  # Symbol
        self.grid_columnconfigure(1, weight=0, minsize=self.MIN_WIDTHS['chart'])    # Chart
        self.grid_columnconfigure(2, weight=1, minsize=self.MIN_WIDTHS['price'])    # Price
        self.grid_columnconfigure(3, weight=1, minsize=self.MIN_WIDTHS['change'])   # Change
        self.grid_columnconfigure(4, weight=0, minsize=self.MIN_WIDTHS['trend'])    # Trend
        self.grid_columnconfigure(5, weight=0, minsize=self.MIN_WIDTHS['signal'])    # Signal
        ...
        self.symbol_lbl = ctk.CTkLabel(self, text=symbol, 
        ...
        self.price_lbl = ctk.CTkLabel(self, text=price, 
                                     font=ctk.CTkFont(family="Consolas", size=font_size), text_color="#FFFFFF")
        self.price_lbl.grid(row=0, column=2, sticky="w", padx=padx, pady=pady)
        
        change_color = "#00FF66" if "+" in change else "#FF1744" if "-" in change else "#8B949E"
        self.change_lbl = ctk.CTkLabel(self, text=change, text_color=change_color, 
        ...
        self.trend_m15 = ctk.CTkLabel(self.trend_frame, text="●", text_color="gray", 
                                      font=ctk.CTkFont(family="Inter", size=trend_dot_size))
        self.trend_m15.pack(side="left", padx=trend_padx)
        self.trend_h1 = ctk.CTkLabel(self.trend_frame, text="●", text_color="gray", 
                                      font=ctk.CTkFont(family="Inter", size=trend_dot_size))
        self.trend_h1.pack(side="left", padx=trend_padx)
        self.trend_h4 = ctk.CTkLabel(self.trend_frame, text="●", text_color="gray", 
                                      font=ctk.CTkFont(family="Inter", size=trend_dot_size))
        ...
        sig_color = "#00FF66" if signal == "BUY" else "#FF1744" if signal == "SELL" else "#8B949E"
        self.signal_btn = ctk.CTkButton(self, text=signal, 
                                       width=sig_width, height=sig_height, 
                                       fg_color=sig_color, hover_color=sig_color, 
                                       text_color="#0B0E14",
                                       corner_radius=10,
    ...
    def update_data(self, price, change, history=None):
        ...
        if history is not None and len(history) > 1:
            ...
            self.ax.set_ylim(min(y) - (max(y)-min(y))*0.1, max(y) + (max(y)-min(y))*0.1)
    ...
    def update_trend(self, m15_color, h1_color, h4_color):
        ...
        if all(c == green for c in colors):
            self.signal_btn.configure(text="BUY", fg_color="#00FF66", hover_color="#00E676", text_color="#0B0E14")
        elif all(c == red for c in colors):
            self.signal_btn.configure(text="SELL", fg_color="#FF1744", hover_color="#D50000", text_color="#0B0E14")
        else:
            self.signal_btn.configure(text="WAIT", fg_color="#FFEA00", hover_color="#FFD600", text_color="#0B0E14")
